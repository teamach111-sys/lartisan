...
<head>
    ...
    <title>Inscription - Lartisan</title>
...
<body class="bg-[#f4f4f0] min-h-screen flex items-center justify-center p-2 sm:p-4">
        ...
        <div class="flex flex-col lg:flex-row bg-[#f4f4f0] overflow-hidden">
            <div class="flex flex-col w-full lg:w-1/2 p-4 sm:p-8 md:p-12">
                <div class="flex justify-between items-center w-full mb-6 sm:mb-10">
                    <a href="{{ url('/') }}">
                        <img class="h-12 sm:h-16" src="{{ asset('imgs/logo.svg') }}" alt="Lartisan Logo">
                    </a>
                    <a class="font-bold border-b border-black hover:text-[#fb663f] hover:border-[#fb663f] transition-colors"
                        href="{{ route('login') }}">Connexion</a>
                </div>

                <div
                    class="bg-white border border-black p-6 sm:p-8 rounded-sm transition-all duration-200 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#000000]">
                    <h1
                        class="text-2xl sm:text-3xl font-black mb-6 sm:mb-8 border-b border-black pb-4 uppercase tracking-tight">
                        Inscription</h1>
                    ...
                    <form action="{{ route('register') }}" method="POST" enctype="multipart/form-data"
                        ...
                        @if ($errors->any())
                            <div
                                class="bg-red-100 border border-red-500 text-red-700 px-4 py-3 rounded-sm mb-6 font-bold text-sm">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                            <div class="flex flex-col gap-1 sm:gap-2">
                                <label class="font-bold text-xs sm:text-sm uppercase tracking-wider" for="nom">Nom
                                    complet</label>
                            ...
                            <div class="flex flex-col gap-1 sm:gap-2">
                                <label class="font-bold text-xs sm:text-sm uppercase tracking-wider"
                            ...
                            <div class="flex flex-col gap-1 sm:gap-2">
                                <label class="font-bold text-xs sm:text-sm uppercase tracking-wider" for="password">Mot
                                    de passe</label>
                            ...
                            <div class="flex flex-col gap-1 sm:gap-2">
                                <label class="font-bold text-xs sm:text-sm uppercase tracking-wider"
                                    for="password_confirmation">Confirmation</label>
                                ...
                                <select name="ville_utilisateur" id="ville" required
                                    ...
                                    @foreach ($villes as $ville)
                                        <option value="{{ $ville->nom }}"
                                            {{ old('ville_utilisateur') == $ville->nom ? 'selected' : '' }}>
                                            {{ $ville->nom }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="flex flex-col items-center gap-2 sm:gap-3">
                            ...
                            <label for="pfp"
                                class="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full border-2 border-dashed border-black bg-gray-50 hover:bg-white hover:border-[#fb663f] transition-all cursor-pointer group overflow-hidden flex items-center justify-center">
                                <img id="pfp-preview"
                                    class="absolute inset-0 w-full h-full object-cover rounded-full hidden">
                                <div id="pfp-placeholder"
                                    class="flex flex-col items-center text-gray-400 group-hover:text-[#fb663f]">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-8 sm:size-10">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <div id="pfp-overlay"
                                    class="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hidden">
                                    <span
                                        class="text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-widest">Changer</span>
                                </div>
                                <input name="pfp" id="pfp" type="file" accept="image/*" class="hidden"
            ...
            <div class="hidden lg:block lg:w-1/2 relative">
                ...
                <img class="w-full h-full object-cover border-l border-black"
                    src="{{ asset('imgs/register_bg.svg') }}" alt="Artisan workshop">
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            btn.disabled = true;
            btn.innerHTML =
                '<span class="flex items-center gap-2 justify-center"><svg class="animate-spin h-5 w-5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Inscription...</span>';
