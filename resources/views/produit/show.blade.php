<x-layout>
    <div class="flex flex-col lg:w-2/3 w-full mx-auto h-full overflow-y-auto py-4 lg:py-9">
        ...
        <div class="mb-6 ml-1">
            <a href="{{ route('home') }}"
                class="inline-flex items-center gap-2 px-4 py-2 border border-black bg-white rounded-sm font-bold transition-all duration-200 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_#000000]">
        ...
        <div class="flex flex-col md:flex-row border">
            ...
            <div class="flex flex-col w-full md:w-2/3 border-r-0 md:border-r border-b md:border-b-0">
                ...
                {{-- Title --}}
                <div
                    class="bg-white p-5 lg:p-6 border-b flex justify-between items-start md:items-center flex-col md:flex-row gap-2 md:gap-0">
                    <h1 class="text-2xl font-bold break-words w-full md:max-w-[70%] leading-tight">{{ $produit->titre }}
                    </h1>
                    <span
                        class="px-3 py-1 bg-gray-100 border border-black/10 rounded-sm text-sm font-bold shrink-0">{{ $produit->ville_produit }}
                        • {{ $produit->created_at->diffForHumans() }}</span>
                </div>
                    ...
                    <div class="p-5 lg:p-6 flex-1 flex items-center">
                        <div class="flex gap-3 items-center">
                            ...
                            <div class="flex flex-col">
                                ...
                                <span
                                    class="text-[10px] font-black uppercase text-black/40">{{ $produit->ville_produit }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white w-full md:w-1/3 p-5 lg:p-6 flex flex-col gap-4">
                    ...
                    <div class="p-4 bg-gray-50 border border-black/5 rounded-sm flex items-center gap-3">
                        <div class="bg-black p-2 rounded-full">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="2.5" stroke="white" class="size-4">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="flex flex-col">
                            <span class="text-[10px] uppercase font-black text-gray-400 leading-none mb-1">Téléphone de
                                Lartisan</span>
                            <a href="tel:{{ $produit->vendeur->telephone }}"
                                class="font-bold text-lg hover:text-[#fb663f] transition-colors leading-none">
                ...
                <div class="flex gap-3 items-center">
                        ...
                        <form action="{{ route('produit.favorite', $produit->id) }}" method="POST" class="flex-1">
                            ...
                            <button type="submit"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    fill="{{ $isFavorited ? '#FF8E72' : 'none' }}" viewBox="0 0 24 24"
                                    stroke-width="2" stroke="{{ $isFavorited ? '#FF8E72' : 'currentColor' }}"
                    ...
                    <button type="button"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                            stroke="currentColor" class="size-6 group-hover:text-[#FF8E72] transition-colors">
                            <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                ...
                                <form action="{{ route('produit.signaler', $produit->id) }}" method="POST">
                                    @csrf
                                    <label class="block text-xs font-black uppercase text-black/40 mb-2">Raison du
                                        signalement</label>
                                    ...
                                    <label class="block text-xs font-black uppercase text-black/40 mb-2">Détails
                                        (facultatif)</label>
                ...
                @if (session('info'))
                    <div
                        class="p-3 bg-yellow-50 border border-yellow-300 rounded-sm text-yellow-800 text-sm font-medium">
                        {{ session('info') }}
                    </div>
                @endif
