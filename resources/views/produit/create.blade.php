<x-layoutdash>
    ...
    <div class="max-w-4xl">
        <form action="{{ route('produit.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div
                class="bg-white border border-black p-6 rounded-sm transition-all duration-200 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input name="prix" id="prix" type="number" value="{{ old('prix') }}" max="9999999999"
                            class="focus:shadow-[0_0_0_2px_#fb663f] outline-none bg-white border border-black rounded-sm h-12 px-4 transition-all duration-200 hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                            placeholder="0.00">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="categorie" id="categorie"
                            ...
                            @foreach ($categories as $cat)
                                <option value="{{ $cat->id }}"
                                    {{ old('categorie') == $cat->id ? 'selected' : '' }}>{{ $cat->nom }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="ville_produit" id="ville_produit"
                            ...
                            @foreach ($villes as $ville)
                                <option value="{{ $ville->nom }}"
                                    {{ old('ville_produit') == $ville->nom ? 'selected' : '' }}>{{ $ville->nom }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <select name="etat_produit" id="etat_produit"
                            class="focus:shadow-[0_0_0_2px_#fb663f] outline-none bg-white border border-black rounded-sm h-12 px-4 transition-all duration-200 hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] cursor-pointer">
                            <option value="premiere_main"
                                {{ old('etat_produit') == 'premiere_main' ? 'selected' : '' }}>Première main / Neuf
                            </option>
                            <option value="occasion" {{ old('etat_produit') == 'occasion' ? 'selected' : '' }}>Occasion
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    @for ($i = 0; $i < 5; $i++)
                        <div class="aspect-square border-2 border-dashed border-black/20 rounded-sm bg-gray-50 flex items-center justify-center relative overflow-hidden group cursor-pointer hover:border-[#fb663f] transition-all"
                            ...
                            <div id="overlay-{{ $i }}"
                                class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hidden">
                                <span
                                    class="text-white text-[10px] font-bold uppercase tracking-widest">Remplacer</span>
                            </div>
                            ...
                            <input name="images[{{ $i }}]" id="photo-{{ $i }}" class="hidden"
                                type="file" accept="image/*"
                                onchange="previewIndividual(this, {{ $i }})">
                        </div>
                    @endfor
                </div>

                <p class="text-xs text-gray-400 mt-2 italic text-center">Cliquez sur chaque case pour ajouter une photo.
                    Format acceptés : JPG, PNG.</p>
            </div>
    ...
    <script>
            ...
            if (file) {
                ...
                if (placeholder) {
                    ...
                    placeholder.innerHTML =
                        '<svg class="animate-spin h-8 w-8 text-[#fb663f]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span class="text-[10px] font-bold mt-1 uppercase text-[#fb663f]">Compression...</span>';
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            if (totalSize > maxTotalSize) {
                alert(
                    `Le poids total des images (${(totalSize / (1024 * 1024)).toFixed(2)} Mo) dépasse la limite autorisée de 10 Mo.`);
                isValid = false;
            }
            ...
            btn.disabled = true;
            btn.innerHTML =
                '<span class="flex items-center gap-2"><svg class="animate-spin h-5 w-5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Publication en cours...</span>';
