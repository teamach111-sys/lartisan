# Laravel Real-Time Messaging System — Full Gist

> Copy-paste ready. Works with Laravel 11+, AlpineJS 3, Laravel Reverb, TailwindCSS v4.
> Replace `produit` / `acheteur` / `vendeur` with your own model names if needed.
...
### User model — add `last_seen_at` migration
```php
...
### `app/Models/User.php` — add to your existing User model
```php
// Add to $fillable:
'last_seen_at',
...
## STEP 4 — Middleware (Online Status Tracking)
